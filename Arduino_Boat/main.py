...
def transmit_telem(currTime):
    if currTime > lora.last_receive + lora.transmission_interval/2:
        if currTime > lora.transmission_interval + lora.last_transmission:
            telem_params = DEF.BridgeCall("get_telemetry")
...
def state_MAN():
    ...
    if params.pwr == False:
        ...
        return
    '''Currently, boat stops if no coast cmd has been sent for 8s'''
    if boat.move_dir >= 0 and curr_time - last_manual_cmd < 8000: # **UPDATE** 
        if boat.move_dir == 0:
            boat.state = FSM.HOLD_POS
            return
...
def state_PWR():
    if stateChange:
        ...
        boat.reset_pids()
        DEF.BridgeCall("set_pwr", False, notify=True)
...
while True:
    try:
        ...
        if (curr_time - lora.last_receive > 15*1e3):
            if boat.state != FSM.RETURN and boat.auto_return == True:
                if params.pwr == False:
                    boat.state = FSM.MOTORS_OFF
                elif (nav.rev_len() > 1 ):
