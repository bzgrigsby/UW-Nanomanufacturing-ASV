...
class Nav_Info:
    ...
    def update_info(self):
        values = DEF.BridgeCall("get_nav_info")
        ...
        if (self.lat_filter is None or self.lng_filter is None):
            self.lat_filter = CoordFilter(raw_lat) # filter using a Kalman filter to smooth GPS bouncing
...
'''
    ...
    * In 100ms, with a top speed of 5 m/s (11mph), the boat can move about 0.5m
    * The GPS has a drift of about 1m in those same 100ms
    * These need to be put into degrees lat lon.
''' 
class CoordFilter:
