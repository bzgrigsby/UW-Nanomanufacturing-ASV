...
void setup() {
  ...
  // LoRa Radio Setup
    ...
    while (!LoRa.begin(LORA_FREQ)) {
      Monitor.println("LoRa init FAILED");
      loraReady = false;
      delay(1000);
    }
  ...
  // Winch Motor Setup
    ...
    Monitor.println("Winch OK");

  // Initialize GPS
    Serial1.begin(GPS_BAUD);
    Bridge.provide("get_nav_info", getNavInfo);
    Monitor.println("GPS Bridge Started\n");
  ...
  // Magnetometer Setup
    Monitor.println("Beginning Magnetometer Setup");
  
    while(!mlx.begin_I2C(0x18, &Wire1)) {
      Monitor.println("No MLX90393 found. Trying again...");
      delay(100);
    }
    Monitor.println("MLX90393 Found");
...
bool transmitMessage() {
  txRequest = false;
  if (txSize == 0) {
    Monitor.println("Buffer Size is 0");
    return false; 
  }
  LoRa.beginPacket();
  addHeaderBytes();  
...
void winchFunction() {
  ...
  if (delta_t > T/10.0) {
    ...
    Monitor.println("Moving winch " + String(depth) + "->" + String(nxtPt));
...
bool py_winchFunc(float A_, float T_, float O_) {
  Monitor.println("Trying to move the winch automagically....");
...
bool generateWinchPoints() {
  ...
  if (pointsDelta/T_delta > MAX_VEL) { 
    Monitor.println("ERROR. Above MAX VEL");
    return false; 
  }
...
void py_setThrusts(int leftPWM, int rightPWM) {
  if (!PWR_Flag) return;
  lThrust_desired = constrain(leftPWM, 1450, 1550);
  rThrust_desired = constrain(rightPWM, 1450, 1550);
}
...
const int MTD = 25; // us Max Thrust Delta
void rampThrusts() {
...
void getHeading() {
  if(mlx.readData(&mx_L, &my_L, &mz_L)) {
    ...
    float X_comp = mx * cos_p + my * sin_r * sin_p + mz * cos_r * sin_p;
...
void loop() {
  ...
  // Battery
    ...
    if (currTime - battMillis >= BATT_INTERVAL) {
      updateSOC();
      battMillis = currTime;
      Monitor.print("Loop  Counts: "); Monitor.println(loopCounter);
      Monitor.println("\tWinch Dir = " + String(winch_move_dir));
      Monitor.println("\tDepth = " + String(depth) + "m -> " + String(points[winch_idx]) + "m  Duration = " + String(lastDuration) + "us");
      Monitor.println("\tPWR = " +  String(PWR_Flag));
      Monitor.println("\tSOC = " + String(soc) + "%");
      Monitor.println("\tThrusts:");
      Monitor.println("\t\tL_thrust = " + String(lThrust) + "  Desired = " + String(lThrust_desired));
      Monitor.println("\t\tR_thrust = " + String(rThrust) + "  Desired = " + String(rThrust_desired));
      loopCounter = 0;
    }
